...
migrate_role() { local component=$1 name=$2 namespace=$3 before="$work/$1.before.json" request="$work/$1.replace.json" after="$work/$1.after.json"; get_live role.rbac.authorization.k8s.io "$name" "$before" "$namespace"; if [[ ! -s "$before" ]]; then ensure "$component" role.rbac.authorization.k8s.io "$name" "$namespace"; return; fi; if check_live "$component" role.rbac.authorization.k8s.io any "$before" 2>/dev/null; then return; fi; python3 -I -B "$validator" role-migration-request --component "$component" "${common[@]}" --live "$before" >"$request"; verify_endpoints; "${kube[@]}" -n "$namespace" replace -f "$request" >/dev/null; get_live role.rbac.authorization.k8s.io "$name" "$after" "$namespace"; python3 -I -B "$validator" role-migration-result --component "$component" "${common[@]}" --before "$before" --after "$after"; }
...
ensure naranjo-site-serviceaccount serviceaccount naranjo-online-reconciler; migrate_role naranjo-site-role flux-release-reconciler naranjo-online; ensure naranjo-site-rolebinding rolebinding.rbac.authorization.k8s.io naranjo-online-reconciler naranjo-online
ensure lidersea-site-serviceaccount serviceaccount lidersea-com-reconciler; migrate_role lidersea-site-role flux-release-reconciler lidersea-com; ensure lidersea-site-rolebinding rolebinding.rbac.authorization.k8s.io lidersea-com-reconciler lidersea-com
...
if [[ ! -s "$source_live" ]]; then [[ $tag == v0.1.43 ]]; render source target >"$work/source.desired.json"; verify_endpoints; "${kube[@]}" -n flux-system create -f "$work/source.desired.json" >/dev/null; get_live gitrepository.source.toolkit.fluxcd.io flux-system "$source_live"; fi
