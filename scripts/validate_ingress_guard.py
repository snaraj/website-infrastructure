...
(`nft -a -j list ruleset`) against one closed expected model — never substring
...
def _parse_rule_expr(expr):
    ...
    counter = expr[2]["counter"]
    if (
        not isinstance(counter, dict)
        or set(counter) != {"packets", "bytes"}
        or any(
            type(counter[key]) is not int
            or counter[key] < 0
            or counter[key] > 2**64 - 1
...
def _validated_ruleset_items(document):
    """Return one current-schema nft list-ruleset envelope or a fixed token."""

    if (
        not isinstance(document, dict)
        or set(document) != {"nftables"}
        or not isinstance(document["nftables"], list)
    ):
        return None, "RULESET_JSON_INVALID"
    items = document["nftables"]
    if any(not isinstance(item, dict) or len(item) != 1 for item in items):
        return None, "RULESET_JSON_INVALID"
    metainfo_indices = [
        index for index, item in enumerate(items) if "metainfo" in item
    ]
    if metainfo_indices != [0]:
        return None, "SCHEMA_VERSION_UNSUPPORTED"
    metainfo = items[0]["metainfo"]
    if (
        not isinstance(metainfo, dict)
        or set(metainfo) != {"version", "release_name", "json_schema_version"}
        or not isinstance(metainfo["version"], str)
        or not metainfo["version"]
        or not isinstance(metainfo["release_name"], str)
        or not metainfo["release_name"]
        or type(metainfo["json_schema_version"]) is not int
        or metainfo["json_schema_version"] != 1
    ):
        return None, "SCHEMA_VERSION_UNSUPPORTED"
    return items, None


def _is_positive_handle(value):
    """Accept exactly one positive kernel uint64 nftables handle."""

    return type(value) is int and 1 <= value <= 2**64 - 1


def _known_object_identity_valid(kind, body):
    """Validate every known object's identity before ownership filtering."""
    ...
    if any(
        ...
        return False
    if not _is_positive_handle(body.get("handle")):
        return False
    return kind != "rule" or type(body.get("expr")) is list
...
def model_errors(document, interfaces):
    ...
    errors = []
    items, envelope_error = _validated_ruleset_items(document)
    if envelope_error:
    ...
    for item in items[1:]:
        ...
        if not _known_object_identity_valid(kind, body):
            return ["RULESET_JSON_INVALID"]
        if kind == "table":
    ...
    if (
        set(table) != {"family", "name", "handle"}
        or not _is_positive_handle(table.get("handle"))
    ):
        errors.append("TABLE_GRAMMAR_UNSUPPORTED")
        return errors
    if table["family"] != OWNED_FAMILY:
    ...
    if (
        set(chain) != required_chain_keys
        or not _is_positive_handle(chain.get("handle"))
    ):
    ...
    if type(chain.get("prio")) is not int or chain.get("prio") != CHAIN_PRIORITY:
    ...
    parsed = []
    owned_rule_handles = set()
    required_rule_keys = {"family", "table", "chain", "handle", "expr"}
    for rule in owned_rules:
        if (
            set(rule) != required_rule_keys
            or not _is_positive_handle(rule.get("handle"))
        ):
        ...
        if rule.get("family") != OWNED_FAMILY or rule.get("chain") != OWNED_CHAIN:
            ...
            continue
        if rule["handle"] in owned_rule_handles:
            errors.append("RULE_HANDLE_DUPLICATE")
            continue
        owned_rule_handles.add(rule["handle"])
...
def _reject_duplicate_json_members(pairs):
    """Keep duplicate object members from disappearing through last-wins."""
...
def _reject_nonstandard_json_constant(_value):
    """Reject the NaN/Infinity extensions accepted by Python's decoder."""

    raise ValueError


def _parse_finite_json_float(value):
    """Admit JSON floats only when their binary representation stays finite."""
...
def _json_depth_within_limit(document):
    """Bound valid-but-hostile nesting without recursive traversal."""

    pending = [(document, 0)]
    while pending:
        value, depth = pending.pop()
        if depth > MAX_RULESET_JSON_DEPTH:
            return False
...
def _read_ruleset(path):
    try:
        ...
        if not _json_depth_within_limit(document):
            raise ValueError
        return document, []
