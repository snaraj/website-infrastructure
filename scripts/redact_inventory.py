...
PATTERNS = [
    ...
    (re.compile(r"AGE-SECRET-KEY-(?:PQ-)?1[A-Z0-9]+"), "[REDACTED_AGE_IDENTITY]"),
    (re.compile(r"\b(?:gh[pousr]_[A-Za-z0-9]{12,}|github_pat_[A-Za-z0-9_]{12,})\b"), "[REDACTED_TOKEN]"),
    (re.compile(r"\b(?:cfk|cfut|cfat)_[A-Za-z0-9]{40}[0-9A-Fa-f]{8}\b"), "[REDACTED_CLOUDFLARE_TOKEN]"),
    (re.compile(r"(?i)(\bcloudflare_api_token\b[\"']?\s*[:=]\s*[\"']?)[A-Za-z0-9_-]{20,}([\"']?)"), r"\1[REDACTED]\2"),
    (re.compile(r"(?i)(authorization:\s*bearer\s+)[^\s]+"), r"\1[REDACTED]"),
    (re.compile(r"(?i)(\b(?:cloudflared[_ -]?)?tunnel[_ -]?token\b[\"']?\s*[:=]\s*[\"']?)(?:eyJ[A-Za-z0-9+/_-]{20,}={0,2}|[A-Za-z0-9+/_-]{80,}={0,2})([\"']?)"), r"\1[REDACTED]\2"),
...
for line in sys.stdin:
    ...
    if re.search(r"-----BEGIN [A-Z0-9 ]*PRIVATE KEY-----", line):
        ...
        if not re.search(r"-----END [A-Z0-9 ]*PRIVATE KEY-----", line):
