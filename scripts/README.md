...
[`workload_registry.py`](./workload_registry.py) parses the one strict, bounded declared-workload registry and proves its exact manifest and generated-Rego closure; [`promote_releases.py`](./promote_releases.py) closes the loop the watchdog opens (issue #286): it derives every promotable workload from that registry and its annotated `OCIRepository` manifest (unknown profiles and any set or identity disagreement refused), runs the issue-195 acquisition ceremony with every judgment in code — double tag resolution with `docker-content-digest` agreement, config and sole Helm layer hash-verified by digest, `Chart.yaml`/`values.yaml` inspection, the embedded workload pin resolved twice and bound to the exact index digest, every declared platform child, cosign verification at the chart digest, SLSA v1 provenance at the index digest, the immutable Release asset hashed against GitHub's stated digest and its `source_sha` bound to the annotated tag's commit — regenerates the receipt and moves only the exact manifest selector plus its byte-exact generated policy projection, then, as the owner's launchd tick, gates, signs, pushes, opens the Draft promotion pull request, arms both review lanes, and flips Ready only on two distinct exact-head adversarial APPROVE receipts. `verify` re-derives the committed receipt from the registry and must reproduce it.
