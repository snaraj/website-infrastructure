...
[`cloudflare-reviewed-launcher.sh`](./cloudflare-reviewed-launcher.sh) is the fixed root-owned macOS entrypoint that admits only an exact fixed-origin live-main tip with matching monotonic bundle history and launcher bytes, a clean environment, pinned tools, and closed operations; [`cloudflare_root_transaction.py`](./cloudflare_root_transaction.py) implements the journaled seven-phase owner-admin transaction, phase-scoped JIT credentials, exact saved-plan execution, revocation proof, complete actor-bound audit-log closure, and crash recovery.
