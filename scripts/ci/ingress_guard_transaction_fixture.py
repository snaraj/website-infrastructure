...
if args[:2] == ["get", "--raw=/readyz"]:
    ...
    # A readiness-only failure keeps later API calls healthy so a dropped
    # production propagation guard cannot be masked by the next command.
    if not configuration.get("api_ready", True):
        raise SystemExit(1)
    print("ok")
    raise SystemExit(0)
if args[:2] == ["get", "nodes"]:
...
if args[:1] == ["rollout"]:
    if "daemonset/calico-node" in args:
        ...
        resource = next(name for name in args if name.startswith("deployment/"))
        controller = resource.split("/", 1)[1]
        record("kubectl:flux-rollout:" + controller)
        if not configuration.get(controller.removesuffix("-controller") + "_ready", True):
            raise SystemExit(1)
    elif any(name in args for name in (
...
if args[:1] == ["get"]:
    ...
    flux_controller = (
        resource.split("/", 1)[1]
        if namespace == "flux-system" and resource.startswith("deployment/")
        else None
    )
    present = {
        ...
        ("flux-system", "deployment/source-controller"): (
            configuration.get("flux", False)
            and configuration.get("source_present", True)
        ),
        ("flux-system", "deployment/kustomize-controller"): (
            configuration.get("flux", False)
            and configuration.get("kustomize_present", True)
        ),
        ("flux-system", "deployment/helm-controller"): (
            configuration.get("flux", False)
            and configuration.get("helm_present", True)
        ),
    ...
    if present:
        if resource == "namespace/flux-system":
            ...
            record("kubectl:flux-present:" + flux_controller)
...
def healthy_ruleset(repo_root):
    ...
    for handle, (iface, port, verdict) in enumerate(
        module.expected_sequence((interface,)), start=100
    ):
        items.append({
            "rule": {
                "family": "inet",
...
def assert_exact_flux_controller_probes(rows):
    expected = [
        verb + controller
        for controller in (
            "source-controller",
        ...
        for verb in ("kubectl:flux-present:", "kubectl:flux-rollout:")
    ]
    probes = []
    current = None
    for row in rows:
        verb = row["verb"]
        if verb == "kubectl:api-ready":
    ...
    if current is not None:
        probes.append(current)
    applicable = [probe for probe in probes if "kubectl:flux-namespace" in probe]
    if not applicable:
        fail("applicable Flux health probe missing")
    for index, probe in enumerate(applicable, start=1):
        observed = [verb for verb in probe if verb.startswith("kubectl:flux-")]
        observed.remove("kubectl:flux-namespace")
        if observed != expected:
            fail(
                "Flux health probe "
                + str(index)
                + " controller sequence mismatch: "
...
def scenario_success(fixture):
    ...
    assert_exact_flux_controller_probes(rows)
    counts = {}
    for row in rows:
...
def scenario_interruption(fixture, phase, mode):
    ...
    if mode == "kill":
        if phase == "prepared":
            fixture.set_control(
                "rollback-intent", "systemctl:daemon-reload", "fail"
            )
            status, _output = fixture.pty_run("--recover")
            if status == 0 or fixture.journal().get("phase") != "recovery-required":
                fail("post-intent recovery failure was not durably reported")
            journal = fixture.journal()
            receipt = Path(
                "/var/lib/website-infrastructure/ingress-guard/receipts"
            ) / f"retrofit.{journal['attempt_id']}.recovery-required.receipt.v2"
            fields = dict(
                line.split("=", 1)
            ...
            expected_receipt = {
                ...
                "custody_receipt_sha256": journal["custody_receipt_sha256"],
                ...
                "guard_state": "unverified",
                "persistence": "unverified",
                "kubelet_dependency": "unverified",
                "cluster_health": "unverified",
                "rollback": "required",
            }
            if fields != expected_receipt:
                fail("recovery-required receipt overstated failure state")
            fixture.clear_control()
        status, _output = fixture.pty_run("--recover")
        if status != 0:
...
def alternate_recovery_bundle(fixture, label, revision):
    source = fixture.scratch / ("alternate-recovery-bundle-" + label)
    copy_public_tree(fixture.repo_root, source)
    recovery_relative = "bootstrap/pi/ingress-guard/recover-ingress-guard.sh"
    recovery = source / recovery_relative
    recovery.write_bytes(
        recovery.read_bytes()
        + ("\n# Hostile fixture: reviewed recovery bundle " + label + ".\n").encode(
            "ascii"
        )
    )
    recovery.chmod(0o755)
    recovery_digest = hashlib.sha256(recovery.read_bytes()).hexdigest()
    manifest = source / "bootstrap/pi/ingress-guard/source-manifest.v1"
    rows = []
    replaced = 0
    for row in manifest.read_text(encoding="ascii").splitlines():
        digest, mode, relative = row.split("\t")
        if relative == recovery_relative:
            digest = recovery_digest
            replaced += 1
        rows.append("\t".join((digest, mode, relative)))
    if replaced != 1:
        fail("alternate recovery manifest did not bind one recovery blob")
    manifest.write_text("\n".join(rows) + "\n", encoding="ascii")
    manifest.chmod(0o644)
    return source, revision, hashlib.sha256(manifest.read_bytes()).hexdigest()


def receipt_snapshot():
    root = Path("/var/lib/website-infrastructure/ingress-guard/receipts")
    return {
...
def operational_snapshot(fixture):
    def content(path):
        return path.read_bytes() if path.exists() else None

    return {
        "ledger": content(fixture.harness / "ledger.jsonl"),
        "systemctl": content(fixture.harness / "systemctl.json"),
        "nft": content(fixture.harness / "nft-state"),
    }
...
def scenario_stale_binding(fixture):
    success_activation(fixture)
    journal_path = Path(
        "/var/lib/website-infrastructure/ingress-guard/transaction/journal.v2"
    )
    alternate_bundles = (
        (
            "same-revision-different-manifest",
            alternate_recovery_bundle(
                fixture, "same-revision", SOURCE_REVISION
            ),
        ),
        (
            "different-revision-and-manifest",
            alternate_recovery_bundle(fixture, "different-revision", "b" * 40),
        ),
    )
    for label, (alternate_source, alternate_revision, alternate_manifest) in (
        alternate_bundles
    ):
        environment = fixture.environment("--stage")
        environment.update(
            {
                "INGRESS_GUARD_SOURCE_ROOT": str(alternate_source),
                "INGRESS_GUARD_SOURCE_REVISION": alternate_revision,
                "INGRESS_GUARD_MANIFEST_SHA256": alternate_manifest,
                "CONFIRM_INGRESS_GUARD_CUSTODY": (
                    "custody-reviewed-ingress-guard-"
                    + alternate_revision
                    + "-"
                    + alternate_manifest
                ),
        ...
        status, output = fixture.pty_run("--stage", environment)
        if status != 0:
            fail(
                label
                + " custody stage failed: "
                + output[-2000:].decode("utf-8", "replace")
            )
        journal_before = journal_path.read_bytes()
        receipts_before = receipt_snapshot()
        operations_before = operational_snapshot(fixture)
        environment = fixture.environment("--recover")
        environment.update(
            {
                "INGRESS_GUARD_SOURCE_REVISION": alternate_revision,
                "INGRESS_GUARD_MANIFEST_SHA256": alternate_manifest,
        ...
        status, _output = fixture.pty_run("--recover", environment)
        if status == 0:
            fail(label + " recovered an older journal")
        if journal_path.read_bytes() != journal_before:
            fail(label + " changed the older journal")
        if receipt_snapshot() != receipts_before:
            fail(label + " changed transaction receipts")
        if operational_snapshot(fixture) != operations_before:
            fail(label + " reached host or API operations")
...
def scenario_receipt_tamper(fixture):
    ...
    receipt.chmod(0o600)
    status, _output = fixture.pty_run("--retrofit-close")
    committed = fixture.journal()
    if status != 0 or committed.get("phase") != "committed":
    ...
    journal_before = journal_path.read_bytes()
    pass_receipt = Path("/var/lib/website-infrastructure/ingress-guard/receipts") / (
        f"retrofit.{committed['attempt_id']}.pass.receipt.v2"
    )
    pass_before = pass_receipt.read_bytes()
    receipts_before = receipt_snapshot()
    operations_before = operational_snapshot(fixture)
    pass_receipt.write_bytes(pass_before + b"tampered=yes\n")
    pass_receipt.chmod(0o600)
    receipts_after_tamper = receipt_snapshot()
    status, _output = fixture.pty_run("--recover")
    if status == 0:
        fail("invalid closed receipt was accepted")
    if journal_path.read_bytes() != journal_before:
        fail("invalid closed receipt demoted the committed journal")
    if receipt_snapshot() != receipts_after_tamper:
        fail("invalid closed receipt changed the receipt set")
    if operational_snapshot(fixture) != operations_before:
        fail("invalid closed receipt reached host or API operations")

    pass_receipt.write_bytes(pass_before)
    pass_receipt.chmod(0o600)
    status, _output = fixture.pty_run("--recover")
    if status != 0 or journal_path.read_bytes() != journal_before:
        fail("restored closed receipt was not an idempotent committed no-op")
    if receipt_snapshot() != receipts_before:
        fail("closed recovery no-op changed transaction receipts")
    if operational_snapshot(fixture) != operations_before:
        fail("closed recovery no-op reached host or API operations")
...
def scenario_unhealthy(fixture):
    ...
    cases = (
        ("readyz-only", {"api_ready": False}, "kubectl:api-ready"),
        ("missing", {"helm_present": False}, "kubectl:flux-present:helm-controller"),
        ("unready", {"helm_ready": False}, "kubectl:flux-rollout:helm-controller"),
        ("api-unavailable", {"healthy": False}, "kubectl:api-ready"),
    )
    for label, override, boundary_verb in cases:
        case_name = (
            "cluster API readiness"
            if label == "readyz-only"
            else (
                "cluster API unavailable"
                if label == "api-unavailable"
                else "Flux helm-controller " + label
            )
        )
        health = dict(base_health)
        health.update(override)
        (fixture.harness / "health.json").write_text(
        ...
        if status == 0 or fixture.journal():
            fail(case_name + " prestate produced a journal")
        if (fixture.harness / "nft-state").exists():
            fail(case_name + " prestate mutated nft")
        rows = [
            json.loads(line)
            for line in ledger_path.read_text(encoding="utf-8").splitlines()[before:]
        ]
        verbs = [row["verb"] for row in rows]
        if "kubectl:call" not in verbs:
            fail(label + " cluster health case did not reach kubectl")
        if label in {"missing", "api-unavailable"} and boundary_verb in verbs:
            fail(label + " cluster health boundary was reported healthy")
        if label == "readyz-only":
            if boundary_verb not in verbs:
                fail("cluster API readiness endpoint was not probed")
            if "kubectl:nodes-listed" in verbs:
                fail("cluster health continued after readiness endpoint failure")
        if label == "unready" and boundary_verb not in verbs:
            fail("unready Flux helm-controller rollout was not probed")
