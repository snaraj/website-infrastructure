...
def post_receipts(workspace: Workspace, github: GitHub, registry: Registry, cosign: Cosign, prs: list, dry_run: bool, run=run_command, outcomes=None) -> None:
    ...
    for pr in prs:
        ...
        try:
            with failure_decision("skip-this-tick"):
                # The review-attention label is RECONCILED, not compensated:
                # every tick reads the head and the label and moves the label
                # to where the receipts say it belongs — retired at a head a
                # receipt binds, armed at a head none binds — so a write a
                # failed or outrun tick left behind converges on the next one
                # instead of needing an undo that could itself fail
                # (2026-09-04 security review rounds 1–3, design reset).
                ...
                if pr["head"] in bound:
                    ...
                    continue
                if not armed and rearm_review_attention(github, pr["number"], pr["head"], dry_run):
                    pr = dict(pr, labels=sorted(set(pr.get("labels", ())) | {READY.REVIEW_ATTENTION_LABEL}))
...
def retire_review_attention(github: GitHub, number: int, head: str, why: str, dry_run: bool) -> bool:
    ...
    here — a compensation is one more write that can fail — but reconciled by
    the next tick, which re-arms a head no receipt binds (see ``post_receipts``).
    Returns whether the label is retired at that head; a failed removal raises,
    and the next tick finishes it.
    """
...
def rearm_review_attention(github: GitHub, number: int, head: str, dry_run: bool) -> bool:
    """Put the review-attention label back on a head no receipt binds.

    The promoter armed this label when it opened the pull request; that is
    its standing authority (AGENTS.md, Agent labels), and re-arming its own
    pull request is the same act. A head without a receipt and without the
    label is a retirement that outran a moved head, or a label somebody
    removed by hand: either way the head is waiting to be judged, and the
    label says so again. The head is re-read immediately before the write.
    """

    live = owned_pull_request(github.api(f"repos/{REPOSITORY}/pulls/{number}"))
    if live is None or live["head"] != head or READY.REVIEW_ATTENTION_LABEL in live["labels"]:
        return False
    if dry_run:
        log(f"PR #{number}: WOULD re-arm {READY.REVIEW_ATTENTION_LABEL} at {head[:7]} (dry run)")
        return False
    github.mutate(f"repos/{REPOSITORY}/issues/{number}/labels", "POST", {"labels": [READY.REVIEW_ATTENTION_LABEL]})
    log(f"PR #{number}: {READY.REVIEW_ATTENTION_LABEL} re-armed at {head[:7]}; no receipt binds this head")
    return True
