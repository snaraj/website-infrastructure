...
NETWORK_BYPASS_FIXTURE = (
    ROOT
    / "tests"
    / "kubernetes"
    / "fixtures"
    / "deny"
    / "admission-install-network-bypasses.yaml"
)
...
# Which admission-namespace flows belong to the admission controller's ROLE and
# which are genuinely shared by every Kyverno controller in the namespace.
# `app.kubernetes.io/part-of: kyverno` is a membership label, not a role label:
# render.lock's `runtime.controllers.names` records that a reports controller
# runs beside the admission controller carrying the same value, so selecting on
# it alone hands that controller webhook ingress it serves nothing on and
# unrestricted public TCP/443 it never uses (both signature policies are
# `background: false`, so it evaluates no `verifyImages` rule).
ROLE_SPECIFIC_FLOWS = ("kyverno-admission-webhook", "kyverno-public-https")
SHARED_FLOWS = ("kyverno-dns", "kyverno-kube-apiserver")
ADMISSION_CONTROLLER_SELECTOR = {
    "app.kubernetes.io/component": "admission-controller",
    "app.kubernetes.io/part-of": "kyverno",
}
SHARED_SELECTOR = {"app.kubernetes.io/part-of": "kyverno"}
...
# --- the kubelet's runAsNonRoot rule, modelled --------------------------------
#
# Image repositories whose OCI config declares no non-root `User`. The
# pre-controller canary's image is constrained to this family by the installer's
# own CANARY_IMAGE_PATTERN, and every `e2e-test-images/agnhost` tag runs as UID 0
# unless the Pod supplies a numeric user.
ROOT_BY_DEFAULT_IMAGE_REPOSITORIES = ("registry.k8s.io/e2e-test-images/agnhost",)


def kubelet_refuses_container(run_as_non_root, run_as_user, image_runs_as_root):
    """Whether the kubelet would refuse to START this container.

    `runAsNonRoot: true` is not a request to pick a non-root user; it is a
    refusal to start a container that resolves to UID 0. The effective UID is
    the container's `runAsUser`, then the Pod's, then the image's own `User`;
    an image that declares none is root.

    This is the property a manifest-shaped assertion cannot see. The API server
    ADMITS such a Pod — the spec is valid — and the kubelet refuses it at
    container start with `container has runAsNonRoot and image will run as
    root`. Against the stubbed kubectl of these tests the create and the wait
    both succeed, so only a model of the runtime rule can turn the defect red.
    """

    if not run_as_non_root:
        return False
    if run_as_user is None:
        return image_runs_as_root
...
class AdmissionNetworkShapeTests(unittest.TestCase):
    ...
    # --- role, not just membership ------------------------------------------
    #
    # These assertions read the committed manifest directly, not the rego's
    # opinion of it: the policy layer is a CI mirror of this contract, and a
    # mirror cannot be the sole guard of the bytes it reflects. A change that
    # widened the manifest and relaxed the rego in the same commit would leave
    # `conftest` green; it stays red here regardless of what the rego says.

    @staticmethod
    def _pod_selector(document):
        block = re.search(
            r"(?m)^  podSelector:\n    matchLabels:\n((?:      \S+: \S+\n)+)", document
        )
        if block is None:
            return {}
        return dict(
            re.findall(r"(?m)^      (\S+): (\S+)$", block.group(1))
        )

    def test_the_role_specific_flows_select_the_admission_controller_alone(self):
        for name in ROLE_SPECIFIC_FLOWS:
            with self.subTest(policy=name):
                self.assertEqual(
                    self._pod_selector(self._document(name)),
                    ADMISSION_CONTROLLER_SELECTOR,
                    "only the admission controller serves the webhook and "
                    "evaluates verifyImages; the shared selector hands this "
                    "flow to every Kyverno controller in the namespace",
                )

    def test_the_genuinely_shared_flows_keep_the_shared_selector(self):
        # The other direction, so the correction cannot be over-applied: DNS and
        # the in-cluster API path are needed by every controller, and narrowing
        # them would break report generation for no security gain.
        for name in SHARED_FLOWS:
            with self.subTest(policy=name):
                self.assertEqual(
                    self._pod_selector(self._document(name)), SHARED_SELECTOR
                )

    def test_the_component_label_is_one_the_reviewed_export_actually_sets(self):
        """Evidence for the selector, not an assumption about it.

        A podSelector naming a label no Pod carries selects nothing, which for
        an ALLOW policy means the admission controller loses the path — a
        narrower selector that is wrong is worse than a broad one that works.
        The reviewed controller export sets both labels on the Deployment's Pod
        TEMPLATE, which is what a podSelector matches.
        """

        export = read(ADMISSION / "kyverno" / "controllers.yaml")
        template = capture(
            r"(?s)\n  template:\n    metadata:\n      labels:\n(.*?)\n      annotations:",
            export,
            "the admission controller's Pod template labels",
        )
        labels = dict(re.findall(r"(?m)^        (\S+): (\S+)$", template))
        for key, value in ADMISSION_CONTROLLER_SELECTOR.items():
            with self.subTest(label=key):
                self.assertEqual(labels.get(key), value)

    @staticmethod
    def _manifest_body(document):
        # The bytes a cluster would see: comments carry the reasoning, never the
        # contract, so a comparison that included them would pass for the wrong
        # reason.
        return "\n".join(
            line
    ...
    def test_the_selector_only_mutations_are_exactly_the_reviewed_shape(self):
        """Two deny documents whose ONLY defect is the selector.

        The pre-existing bypass documents for these two policies are denied for
        a widened peer or a dropped exclusion, so they would stay denied even
        if the selector requirement were reverted — they cannot prove the new
        arm kills anything. These two are byte-identical to the committed
        policy once the narrow selector is substituted back in, which is what
        makes their denial attributable to the selector and nothing else. The
        substitution is performed here rather than asserted in prose, so a
        later edit that quietly widens a peer in the fixture stops being a
        selector proof and fails.
        """

        documents = re.split(r"(?m)^---\s*$", read(NETWORK_BYPASS_FIXTURE))
        for name in ROLE_SPECIFIC_FLOWS:
            with self.subTest(policy=name):
                reviewed = self._manifest_body(self._document(name))
                narrowed = [
                    body
                    for body in (self._manifest_body(d) for d in documents)
                    if re.search(r"(?m)^  name: " + re.escape(name) + r"\s*$", body)
                    and body.replace(
                        "      app.kubernetes.io/part-of: kyverno",
                        "      app.kubernetes.io/component: admission-controller\n"
                        "      app.kubernetes.io/part-of: kyverno",
                ...
                self.assertEqual(
                    len(narrowed),
                    1,
                    "the deny fixture must carry exactly one document for {} "
                    "that differs from the committed policy in the selector "
                    "alone".format(name),
                )

    def test_the_selector_only_denials_are_pinned_in_the_expected_set(self):
        # The `.expected` sidecar is compared as a whole set, so the two new
        # denials have to be recorded there or the fixture runner reports an
        # unreviewed denial.
        expected = read(
            NETWORK_BYPASS_FIXTURE.with_suffix(".expected")
        ).splitlines()
        for name in ROLE_SPECIFIC_FLOWS:
            with self.subTest(policy=name):
                self.assertEqual(
                    expected.count(
                        "NetworkPolicy kyverno/{} widens its exact reviewed "
                        "admission flow".format(name)
                    ),
                    2,
                )

    @unittest.skipUnless(CONFTEST, "conftest is required")
    def test_a_role_specific_flow_with_only_the_shared_selector_is_denied(self):
        completed = subprocess.run(
            [
        ...
        for name in ROLE_SPECIFIC_FLOWS:
            with self.subTest(policy=name):
                ...
                self.assertEqual(output.count(message), 2, output)
...
class RenderLockTests(unittest.TestCase):
    ...
    def test_the_inventory_names_are_the_namespaced_objects(self):
        """The list finding P1-1 showed was missing.

        Without it a namespaced journal row was bound to the NAMESPACE rather
        than to a reviewed identity, and the namespace holds far more than this
        transaction created.
        """

        names = lock_value("inventory.namespaced.names").split(",")
        self.assertEqual(len(names), int(lock_value("inventory.namespaced")))
        self.assertEqual(len(names), len(set(names)), "no identity may be listed twice")
        for name in names:
            with self.subTest(name=name):
                # `Kind/namespace/name`, the journal's own triple, so the
                # membership test cannot be satisfied by a kind/name pair that
                # belongs to a different namespace.
                self.assertRegex(name, r"^[A-Za-z][A-Za-z0-9]*/kyverno/[a-z0-9.-]+$")

    def test_both_inventory_lists_are_disjoint_and_duplicate_free(self):
        cluster = lock_value("inventory.cluster-scoped.names").split(",")
        namespaced = lock_value("inventory.namespaced.names").split(",")
        self.assertEqual(len(cluster), len(set(cluster)))
        self.assertEqual(set(cluster) & set(namespaced), set())
        self.assertEqual(
            len(cluster) + len(namespaced),
            int(lock_value("report-only.objects")),
            "the two lists together are the whole render, and that equality is "
            "what makes a journal entry count equivalent to completeness",
        )
        self.assertEqual(
            lock_value("report-only.objects"),
            lock_value("enforce.objects"),
            "one inventory governs both stages",
        )
    ...
    @unittest.skipUnless(PINNED_RENDERER, NEEDS_RENDERER)
    def test_both_inventory_lists_are_the_identities_the_tree_renders(self):
        """The lists are a delete program's allowlist, so they are re-derived.

        A digest match proves the BYTES are the reviewed ones; it says nothing
        about whether the two name lists still enumerate them. Drift in either
        direction matters: an identity the render creates and the list omits is
        one rollback refuses to remove, and an identity the list carries and the
        render does not is a delete this transaction is not entitled to issue.
        """

        for stage in ("report-only", "enforce"):
            cluster, namespaced = [], []
            for document in re.split(r"(?m)^---\s*$", render(stage)):
                match = re.search(r"(?m)^kind: (\S+)$", document)
                if match is None:
                    continue
                kind = match.group(1)
                name = capture(
                    r"(?m)^  name: (\S+)$", document, "a rendered object name"
                )
                namespace = re.search(r"(?m)^  namespace: (\S+)$", document)
                if namespace is None:
                    cluster.append("{}/{}".format(kind, name))
                else:
                    namespaced.append(
                        "{}/{}/{}".format(kind, namespace.group(1), name)
                    )
            with self.subTest(stage=stage):
                self.assertEqual(
                    sorted(cluster),
                    sorted(lock_value("inventory.cluster-scoped.names").split(",")),
                )
                self.assertEqual(
                    sorted(namespaced),
                    sorted(lock_value("inventory.namespaced.names").split(",")),
                )
...
class RunbookTests(unittest.TestCase):
    ...
    # --- the report-only acceptance contract must be satisfiable -------------
    #
    # The runbook required `failurePolicy: Ignore` on EVERY exact runtime
    # webhook. The report-only overlay patches ClusterPolicy fields, and only
    # Kyverno's RESOURCE webhooks are derived from them; the rest of the locked
    # inventory is constructed by the controller for its own API groups and is
    # unreachable from a ClusterPolicy. An acceptance step that cannot be
    # satisfied is not a gate — it is a step the operator learns to override,
    # which is worse than no step at all.

    def _webhook_matrix(self):
        """The per-webhook expectation table, as {name: expected}."""

        return {
            match.group(1): match.group(2)
            for match in re.finditer(
                r"(?m)^\| `([a-z0-9-]+)` \| `?([A-Za-z]+)`? \|", self.text
            )
        }

    def _reviewed_webhook_names(self):
        names = []
        for kind in ("validating", "mutating"):
            names.extend(lock_value("runtime.webhooks." + kind).split(","))
        return names

    def test_the_acceptance_matrix_covers_every_reviewed_webhook_name(self):
        matrix = self._webhook_matrix()
        for name in self._reviewed_webhook_names():
            with self.subTest(name=name):
                self.assertIn(
                    name,
                    matrix,
                    "the acceptance step states no expectation for a webhook the "
                    "lock says Kyverno registers, so the operator has nothing to "
                    "compare the live object against",
                )
        self.assertEqual(
            sorted(matrix),
            sorted(self._reviewed_webhook_names()),
            "the matrix and render.lock's reviewed inventory must be the same set",
        )

    def test_only_the_resource_webhooks_are_required_to_be_ignore(self):
        """WHICH rows are `Ignore` is the whole content of the correction.

        `Ignore` is achievable exactly where the overlay reaches: the two
        resource webhooks Kyverno builds from the reviewed ClusterPolicies'
        `spec.webhookConfiguration.failurePolicy`.
        """

        matrix = self._webhook_matrix()
        self.assertEqual(
            sorted(name for name, expected in matrix.items() if expected == "Ignore"),
            [
                "kyverno-resource-mutating-webhook-cfg",
                "kyverno-resource-validating-webhook-cfg",
            ],
        ...
        for name, expected in matrix.items():
            with self.subTest(name=name):
                self.assertIn(expected, ("Ignore", "Fail", "absent"))

    def test_the_stop_conditions_bound_a_fail_row_by_its_rules_not_its_policy(self):
        # A `Fail` row is tolerable only because its rules cannot reach a
        # tenant write. That bound is the load-bearing acceptance criterion and
        # has to be stated as a stop, not implied by the table.
        self.assertIn("wgpolicyk8s.io", self.text)
        self.assertIn("kyverno.io", self.text)
        self.assertIn("is a stop regardless of its failure", self.text)

    def test_the_runbook_no_longer_demands_ignore_across_every_webhook(self):
        # The exact unsatisfiable sentence, pinned so it cannot come back.
        self.assertNotIn("timeout, and\n`failurePolicy: Ignore`", self.text)
        self.assertNotIn("Only Audit/Ignore is acceptable", self.text)


class PolicyDirectoryReadmeTests(unittest.TestCase):
    """`policies/kyverno/README.md` must describe the bytes beside it.

    It claimed the signature policies "remain explicitly staged in `Audit`"
    while both sources declare `validationFailureAction: Enforce` and the
    release gate denies a signature policy that reads anything else. A reader
    who trusted the README would conclude the committed set was already
    fail-open and that promoting it was an edit to this directory — it is
    neither. Staging lives entirely in the install overlay, and these
    assertions bind the prose to the two places the truth actually is.
    """

    def setUp(self):
        self.readme = read(POLICIES / "README.md")
        # Paragraphs, not lines: the claim is a sentence-level attribution and
        # the file is hard-wrapped, so a line-scoped check would be trivially
        # satisfied by moving a word to the next line.
        self.paragraphs = [
            " ".join(block.split())
    ...
    def test_the_committed_signature_policies_are_enforce_and_fail_closed(self):
        """The premise the prose is measured against, read from the sources."""

        for policy in SIGNATURE_POLICIES:
            with self.subTest(policy=policy):
                source = read(POLICIES / (policy + ".yaml"))
                self.assertIn("\n  validationFailureAction: Enforce\n", source)
                self.assertIn("\n    failurePolicy: Fail\n", source)
                self.assertNotIn("validationFailureAction: Audit", source)

    def test_the_readme_names_both_signature_policies_as_enforce(self):
        # The old wording named neither and called them Audit. Naming them is
        # what makes the statement checkable against the two files above.
        for policy in SIGNATURE_POLICIES:
            with self.subTest(policy=policy):
                self.assertIn(policy, self.readme)
        naming = [
            paragraph
            for paragraph in self.paragraphs
            if all(policy in paragraph for policy in SIGNATURE_POLICIES)
        ]
        self.assertEqual(
            len(naming),
            1,
            "exactly one paragraph states the signature policies' committed action",
        )
        self.assertIn("Enforce", naming[0])
        self.assertNotIn("Audit", naming[0])

    def test_every_audit_claim_is_attributed_to_the_install_overlay(self):
        """`Audit` is only ever produced by `admission-install/report-only`.

        A paragraph that says `Audit` without naming that overlay is the exact
        defect: it reads as a property of the committed policy set.
        """

        mentions = [
            paragraph for paragraph in self.paragraphs if "Audit" in paragraph
        ]
        self.assertTrue(mentions, "the README must still explain the staged downgrade")
        for paragraph in mentions:
            with self.subTest(paragraph=paragraph[:60]):
                self.assertIn("admission-install/report-only", paragraph)

    def test_the_readme_does_not_promise_a_promotion_by_editing_these_files(self):
        # Promotion is a change of installed STAGE, gated by the installer and
        # render.lock. The old text described a future edit here instead.
        self.assertNotIn("staged in `Audit`", self.readme)
        self.assertIn("render.lock", self.readme)
        self.assertIn("--stage enforce", self.readme)
...
SYNTHETIC_CLUSTER_SCOPED_KINDS = frozenset(
    {"Namespace", "CustomResourceDefinition", "ClusterPolicy"}
)


def _synthetic_inventory():
    """The synthetic render's identities, in the two shapes that matter.

    Derived from the documents rather than transcribed beside them: the journal
    the installer writes for this render is derived from the same documents, so
    a transcribed list could disagree with the render and turn a real refusal
    into a green test.
    """
    ...
    for document in SYNTHETIC_DOCUMENTS:
        kind = capture(r"(?m)^kind: (\S+)$", document, "the synthetic object kind")
        name = capture(r"(?m)^  name: (\S+)$", document, "the synthetic object name")
        if kind in SYNTHETIC_CLUSTER_SCOPED_KINDS:
            journal.append("{}||{}".format(kind, name))
        else:
            namespaced.append("{}/kyverno/{}".format(kind, name))
            journal.append("{}|kyverno|{}".format(kind, name))
    return ",".join(namespaced), "".join(line + "\n" for line in journal)


# The reviewed namespaced identity list the synthetic lock records, and the
# COMPLETE journal body the installer writes for this render. Every accepting
# rollback test uses the complete body: a journal that is not the whole reviewed
# inventory is refused, so a one-line fixture would prove only that the refusal
# fires, never that a real rollback still works.
SYNTHETIC_NAMESPACED, SYNTHETIC_JOURNAL_BODY = _synthetic_inventory()
...
class InstallerGuardTests(unittest.TestCase):
        ...
        (cls.repo / "kubernetes" / "platform" / "admission-install" / "render.lock").write_text(
            "\n".join(
                [
                    ...
                    "inventory.namespaced.names=" + SYNTHETIC_NAMESPACED,
    ...
    def _write_bound_journal(
        ...
        """Write the v3 provenance header before intentionally mutated entries.

        `entries` defaults to the COMPLETE reviewed inventory, because that is
        the only body a real attempt of this transaction produces. A test that
        wants a specific per-line refusal passes the shorter body it needs; a
        test that wants the rollback to SUCCEED must not, or it proves only that
        the completeness refusal fires.
        """

        entries = SYNTHETIC_JOURNAL_BODY if entries is None else entries
    ...
    def _canary_manifest(self):
        """The exact Pod bytes the installer handed `kubectl create`."""

        journal = self.base / "canary-manifest.journal"
        completed = self._run(
            "--stage", "report-only", "--apply", "--journal", str(journal)
        )
        self.assertEqual(completed.returncode, 0, completed.stderr + completed.stdout)
        ...
        journal.unlink()
        self.assertIn("kind: Pod", manifest)
        return manifest

    def test_the_kubelet_model_refuses_and_admits_on_the_inputs_that_matter(self):
        """The model is a guard, so it has to be shown failing.

        Row 2 is the shipped defect and row 3 is the fix; rows 1 and 4 keep the
        model from being a constant.
        """

        for run_as_non_root, run_as_user, image_root, refused in (
            (False, None, True, False),
            (True, None, True, True),
            (True, 10001, True, False),
            (True, 0, False, True),
        ):
            with self.subTest(run_as_non_root=run_as_non_root, run_as_user=run_as_user):
                self.assertEqual(
                    kubelet_refuses_container(run_as_non_root, run_as_user, image_root),
                    refused,
                )

    def test_the_precontroller_canary_can_actually_start_under_its_pinned_image(self):
        """The canary set `runAsNonRoot: true` with no numeric user.

        `agnhost` declares no non-root `User`, so the kubelet resolved UID 0 and
        refused the container after the API server had admitted the Pod. The
        canary would then never reach Succeeded, the wait would report the
        DNS/API path as broken on a cluster whose path was fine, and the
        transaction would roll back a healthy install for a reason nothing in
        its output names.
        """

        manifest = self._canary_manifest()
        # One container, so the security fields below are unambiguous without a
        # YAML parser; a second container would silently escape this read.
        self.assertEqual(manifest.count("    - name: "), 1, manifest)
        image = capture(r"(?m)^      image: (\S+)$", manifest, "the canary image")
        repository = image.split(":", 1)[0]
        self.assertIn(
            repository,
            ROOT_BY_DEFAULT_IMAGE_REPOSITORIES,
            "the canary left the image family this rule was reviewed against; "
            "re-derive whether the new image declares a non-root user",
        )
        run_as_non_root = "runAsNonRoot: true" in manifest
        self.assertTrue(run_as_non_root, "the canary must still refuse to run as root")
        users = re.findall(r"(?m)^\s+runAsUser: (\d+)$", manifest)
        groups = re.findall(r"(?m)^\s+runAsGroup: (\d+)$", manifest)
        self.assertEqual(len(users), 1, "exactly one numeric user must be declared")
        self.assertEqual(len(groups), 1, "exactly one numeric group must be declared")
        self.assertNotEqual(users[0], "0")
        self.assertNotEqual(groups[0], "0")
        self.assertFalse(
            kubelet_refuses_container(run_as_non_root, int(users[0]), True),
            "the kubelet would refuse to start this canary",
        )

    def test_the_precontroller_canary_is_selected_by_the_role_specific_flows(self):
        """The canary is the only Pod the role-specific policies must reach.

        It runs BEFORE the controller exists, in a namespace already closed by
        `default-deny`, and its whole job is to prove the API path those very
        policies grant. Narrowing the webhook-ingress and public-HTTPS selectors
        to `app.kubernetes.io/component: admission-controller` therefore has a
        second failure mode besides being too broad: a canary that does not
        carry the narrowed label is selected by neither policy, fails its dial,
        and rolls back a healthy install. These are the emitted bytes, not the
        heredoc's source, so a future substitution into the label block is read
        the way the cluster would read it.
        """

        labels = capture(
            r"(?s)\nmetadata:\n(?:  \S+: \S+\n)*  labels:\n((?:    \S+: \S+\n)+)",
            self._canary_manifest(),
            "the pre-controller canary's Pod labels",
        )
        carried = dict(re.findall(r"(?m)^    (\S+): (\S+)$", labels))
        for key, value in ADMISSION_CONTROLLER_SELECTOR.items():
            with self.subTest(label=key):
                self.assertEqual(
                    carried.get(key),
                    value,
                    "the canary must match every role-specific NetworkPolicy "
                    "that grants the path it is dialing",
                )
    ...
    def test_rollback_refuses_an_enforce_journal_without_deleting(self):
        ...
        self._write_bound_journal(journal, stage="enforce")
        completed = self._run("--rollback", "--journal", str(journal))
        self.assertNotEqual(completed.returncode, 0)
    ...
    def test_rollback_refuses_a_journal_bound_to_another_render(self):
        ...
        self._write_bound_journal(journal, render_digest="c" * 64)
        completed = self._run("--rollback", "--journal", str(journal))
        self.assertNotEqual(completed.returncode, 0)
    ...
    def test_rollback_refuses_a_journal_bound_to_another_target(self):
        ...
        self._write_bound_journal(journal, target_digest="d" * 64)
        completed = self._run("--rollback", "--journal", str(journal))
        self.assertNotEqual(completed.returncode, 0)
    ...
    def test_rollback_refuses_a_journal_bound_to_another_network_contract(self):
        ...
        self._write_bound_journal(journal, network_digest="e" * 64)
        completed = self._run("--rollback", "--journal", str(journal))
        self.assertNotEqual(completed.returncode, 0)
    ...
    # --- the namespace is a bound, never an identity -------------------------

    def _substituted_journal(self, path, original, substitute):
        """The complete inventory with exactly one identity swapped out.

        Cardinality is preserved deliberately. A hostile journal that is merely
        SHORT is refused by the completeness check, which would prove nothing
        about the membership check this fixture exists to drive.
        """

        self.assertIn(original, SYNTHETIC_JOURNAL_BODY)
        body = SYNTHETIC_JOURNAL_BODY.replace(original + "\n", substitute + "\n")
        self.assertNotEqual(body, SYNTHETIC_JOURNAL_BODY)
        self.assertEqual(
            body.count("\n"),
            SYNTHETIC_JOURNAL_BODY.count("\n"),
            "the substitution must not change the entry count",
        )
        return self._write_bound_journal(path, body)

    def test_rollback_refuses_a_same_namespace_identity_the_render_never_created(self):
        """Finding P1-1: a namespaced row was bound to the NAMESPACE, not the
        reviewed inventory.

        `validate_journal` checked exact `Kind/name` membership only for
        cluster-scoped rows. A namespaced row needed nothing but a kind some
        phase applies and `namespace == kyverno` — and every one of the
        substitutions below satisfies exactly that while naming an object this
        transaction never created. `ServiceAccount|kyverno|default` is the
        namespace's own default identity; `ConfigMap|kyverno|kube-root-ca.crt`
        is the control plane's CA distribution point, recreated by a controller
        and pointed at by every ServiceAccount token in the namespace. Both were
        accepted, and `rollback_journal` issued the delete.
        ...
        and the runtime webhook sweep runs before the first journaled delete.
        """

        for substitute in (
            "ServiceAccount|kyverno|default",
            "ConfigMap|kyverno|kube-root-ca.crt",
            "Service|kyverno|kubernetes",
            "ResourceQuota|kyverno|capacity-not-ready",
            "NetworkPolicy|kyverno|allow-all",
            "Deployment|kyverno|kyverno-reports-controller",
        ):
            with self.subTest(substitute=substitute):
                journal = self.base / "substituted.journal"
                self._substituted_journal(
                    journal, "ConfigMap|kyverno|kyverno", substitute
                )
                completed = self._run("--rollback", "--journal", str(journal))
                self.assertNotEqual(completed.returncode, 0)
                self.assertIn(
                    "which render.lock's reviewed inventory does not contain",
                    completed.stderr,
                )
                self.assertIn(
                    "a same-namespace identity this install never created",
                    completed.stderr,
                )
                self.assertEqual(
                    self._deletes(),
                    [],
                    "a same-namespace substitution must be refused before ANY "
                    "delete, including the runtime webhook sweep",
                )
                journal.unlink()

    def test_rollback_refuses_a_journal_that_repeats_an_identity(self):
        """Uniqueness, with the entry count held constant.

        Membership and cardinality together still accept a journal that names
        one reviewed identity twice and another not at all — so the delete
        program would silently leave an installed object standing while
        reporting a complete rollback.
        """

        journal = self.base / "repeated.journal"
        self._substituted_journal(
            journal, "ConfigMap|kyverno|kyverno", "Service|kyverno|kyverno-svc"
        )
        completed = self._run("--rollback", "--journal", str(journal))
        self.assertNotEqual(completed.returncode, 0)
        self.assertIn("repeats Service/kyverno/kyverno-svc", completed.stderr)
        self.assertEqual(self._deletes(), [])
        journal.unlink()

    def test_rollback_refuses_a_journal_that_is_not_the_complete_inventory(self):
        """Cardinality, with every remaining line a reviewed member.

        A truncated journal is a delete program someone shortened: every line
        passes membership, the rollback reports success, and the objects the
        removed lines named stay installed. One reviewed line — the shape every
        earlier fixture in this class used — is the extreme case.
        """

        for body in (
            SYNTHETIC_JOURNAL_BODY.replace("ConfigMap|kyverno|kyverno\n", ""),
            SYNTHETIC_JOURNAL_BODY.replace("ClusterPolicy||beta\n", ""),
            "ClusterPolicy||alpha\n",
        ):
            with self.subTest(entries=body.count("\n")):
                journal = self.base / "incomplete.journal"
                self._write_bound_journal(journal, body)
                completed = self._run("--rollback", "--journal", str(journal))
                self.assertNotEqual(completed.returncode, 0)
                self.assertIn(
                    "is not the COMPLETE reviewed inventory", completed.stderr
                )
                self.assertEqual(self._deletes(), [])
                journal.unlink()

    def test_rollback_removes_every_reviewed_identity_exactly_once(self):
        """The green half: the complete inventory still rolls back.

        A binding this strict is only worth having if the real journal still
        passes it, so the accepted path is asserted as a set equality over the
        deletes rather than a spot check.
        """

        journal = self.base / "complete.journal"
        self._write_bound_journal(journal)
        completed = self._run("--rollback", "--journal", str(journal))
        self.assertEqual(completed.returncode, 0, completed.stderr + completed.stdout)
        self.assertIn("the complete reviewed inventory exactly once", completed.stdout)
        deleted = []
        for line in self._deletes():
            # Journaled deletes name a capitalized KIND; the runtime webhook
            # sweep — which is asserted by its own tests — passes the lowercase
            # resource name, so the leading capital is what separates them.
            match = re.search(
                r"(?:-n (\S+) )?delete ([A-Z][A-Za-z]*) (\S+) --ignore-not-found", line
            )
            if match is None:
                continue
            ...
            deleted.append(
                "{}/{}".format(kind, name)
                if namespace is None
                else "{}/{}/{}".format(kind, namespace, name)
            )
        expected = SYNTHETIC_CLUSTER_SCOPED.split(",") + SYNTHETIC_NAMESPACED.split(",")
        self.assertEqual(sorted(deleted), sorted(expected))
        self.assertEqual(len(deleted), len(set(deleted)), "no identity deleted twice")
        journal.unlink()

    def test_rollback_refuses_a_lock_whose_inventory_list_and_count_disagree(self):
        """The lock is proven against itself before a journal is measured.

        Cardinality is what turns membership into completeness, so a lock whose
        list is longer than its count would let a SUBSET of the inventory
        satisfy the count check and delete a prefix of the install.
        """

        for key, value in (
            ("inventory.namespaced", "10"),
            ("inventory.cluster-scoped", "3"),
        ):
            with self.subTest(key=key):
                journal = self.base / "lock-mismatch.journal"
                self._write_bound_journal(journal)
                lock = (
                    self.repo
                    / "kubernetes"
                ...
                try:
                    ...
                    lock.write_text(original, encoding="utf-8")
                self.assertNotEqual(completed.returncode, 0)
                self.assertIn("inventory list and count disagree", completed.stderr)
                self.assertEqual(self._deletes(), [])
                journal.unlink()
