...
EXPECTED_INTERFACES = {
    ...
    "gh-pr-flow": (
        "GitHub PR Flow",
        "Owner-only merge flow with exact-head review and releases",
        "Use $gh-pr-flow to handle this GitHub change with issue-first "
        "metadata, isolated append-only authoring, exact-head adversarial "
        "review, an enforced release consequence, and absolute NEVER "
        "MERGE authority.",
    ),
}
...
class SkillStructureTests(unittest.TestCase):
    ...
    def test_pr_flow_pins_ephemeral_recreation_transaction(self):
        ...
        for fragment in (
            ...
            "protected durable state never inherits ephemeral deletion permission",
        ...
        def durable_boundary_denial(text):
            required = (
                "never enter this method's deletion allowlist",
                "protected durable state never inherits ephemeral deletion permission",
            )
            forbidden = (
                "durable state from deletion unless",
                "separately authorized destructive drill",
                "names the accepted loss",
            )
            if any(fragment not in text for fragment in required):
                return "durable-state exclusion is incomplete"
            if any(fragment in text for fragment in forbidden):
                return "durable-state deletion exception is forbidden"
            return None

        self.assertIsNone(durable_boundary_denial(main))
        for index, mutant in enumerate(
            (
                main.replace(
                    "never enter this method's deletion allowlist",
                    "may enter this method's deletion allowlist",
                ),
                main.replace(
                    "protected durable state never inherits ephemeral deletion permission",
                    "protected durable state inherits ephemeral deletion permission",
                ),
                main + " separately authorized destructive drill",
            )
        ):
            with self.subTest(durable_boundary_mutant=index):
                self.assertIsNotNone(durable_boundary_denial(mutant))
    ...
    def test_role_compatibility_and_identity_doctrine_are_consistent(self):
        ...
        def role_boundary_denial(flow_text, contract_text):
            ...
            forbidden = (
                "if no separate coordinator exists",
                "owner may perform that coordination action",
                "coordinator fallback",
            )
            for document in (flow_text, contract_text):
                if any(fragment not in document for fragment in required):
                    return "role separation is incomplete"
                if any(fragment in document for fragment in forbidden):
                    return "owner-as-coordinator fallback is forbidden"
            return None

        self.assertIsNone(role_boundary_denial(flow, contract))
        for index, (flow_mutant, contract_mutant) in enumerate(
            (
                (
                    flow.replace(
                        "coordinator context is distinct from both author and reviewer",
                ...
                (flow + " if no separate coordinator exists", contract),
                (flow, contract + " owner may perform that coordination action"),
            )
        ):
            with self.subTest(role_boundary_mutant=index):
                self.assertIsNotNone(
                    role_boundary_denial(flow_mutant, contract_mutant)
                )
