...
| --- | --- | --- | --- |
| Crown-jewel | age identity, Kubernetes CA private keys, API-encryption keys, media originals, legacy wallet/signing material and private descriptors, anonymity-service identity keys, backup key, MFA recovery codes | Never; media and legacy archive bytes also never enter Git/OCI/etcd | encrypted, separate locations, checksummed, tested recovery |
