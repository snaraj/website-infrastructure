...
`make check` runs the same layers end to end (plus opt-in
...
| --- | --- | --- |
| Layout/privacy/media/secret/workflow/Kubernetes/Cloudflare/activation validators | `validate_repository.py` (8 modes) in CI + `validate-security.sh` | LIVE |
...
| Kyverno allow **and** deny cases | `kyverno test tests/kubernetes/kyverno` (52 cases) + `test-policy-fixtures.sh` | LIVE |
