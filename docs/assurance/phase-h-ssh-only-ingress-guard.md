...
contract → capture `nft -a -j` → require owned identity ABSENT (a same-named decoy
...
Recovery additionally binds the selected sealed entrypoint/library bundle to
the journal's exact revision, manifest, custody directory, and custody-receipt
hash before it uses journal-selected custody or reaches a host/API operation.
Pre-mutation mismatch, malformed state, or invalid closed receipt leaves the
prior journal and receipt set unchanged. Only a durably published
`rollback-intent` arms a later `recovery-required` transition, whose receipt
does not claim current boot, cluster, or private-contract verification.
...
Input is `nft -a -j list ruleset` JSON normalized against one expected model;
text substring matching is never used. The envelope is exactly `{nftables}`
and starts with exactly one metainfo object whose exact keys are
`{version,release_name,json_schema_version}`, whose two strings are nonempty,
and whose schema version is the integer `1`. Allowed object keys: table
`{family,name,handle}`, chain
`{family,table,name,handle,type,hook,prio,policy}`, rule
`{family,table,chain,handle,expr}`; rule expressions exactly
`[iifname ==, tcp dport ==, counter, accept|drop]`. Object handles are exact
positive uint64 integers, owned-rule handles are distinct, priority is the
integer `-10`, and each captured anonymous counter is exactly
`{packets,bytes}` with both values in the uint64 range. Raw JSON with duplicate
members, non-finite numbers, or excessive nesting is rejected before model
selection, and every known table/chain/rule has a valid minimal identity before
ownership filtering. Everything else
— including every table flag (`dormant`, `owner`, or `persist`), inversions,
sets, ranges, prefixes, wildcards, verdict maps, jumps, foreign objects in the
owned table, same-named decoy chains, duplicate tables, single-family variants,
unknown or missing keys, floats, and boolean/null encodings — is a fixed failure
token. Diagnostics are value-free by construction
...
- Kernel-level packet behavior, reboot persistence, sandbox viability of the
  ...
  items above. Offline proof covers manifest closure, held-inode copy logic,
   ...
   states, cross-bundle recovery refusal, closed-journal preservation, truthful
   failure receipts, rollback, and zero-temporary-residue proofs; it still does
   not claim those live outcomes.
