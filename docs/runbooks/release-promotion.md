...
immediately before the one command that posts. Posting either verdict also
removes `requires-review`, as the Verdict format in `AGENTS.md` requires. The
label is reconciled rather than compensated: every tick reads the head and
the label and moves the label to where the receipts say it belongs — retired
at a head a receipt binds, re-armed at a head none binds — so a removal a
failed tick left undone, or one that outran a moved head, converges on the
next tick instead of needing an undo that could itself fail. Every write is
