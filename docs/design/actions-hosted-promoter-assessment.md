...
This record was authored on #291 (head
`7bcf6de4996f12eb92bf116709a739213939533f`, base `2828d250`) and re-cut onto
`main` at `f3e8407a` as #293 after #292 merged the workstation promoter. The
content comparison that matters is restricted to the two files #291 owns,
run from the #293 branch:

```sh
git diff 7bcf6de4996f12eb92bf116709a739213939533f HEAD -- \
  changelog.d/289-actions-promoter-assessment.md \
  docs/design/actions-hosted-promoter-assessment.md
```

Its scope is the sentence above naming the shipped promoter as the supported
path, plus this section. An unrestricted comparison of the two heads also
carries the merged #292 because the base moved; that is base movement, not
assessment content. The NOT-YET verdict and the six conditions are unchanged
from #291.
