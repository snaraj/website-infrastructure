...
| Flux site desired state | Combined #195/#189 candidate: both manifests are unsuspended at exact signed chart digests behind the protected-main-branch source (owner decoupling ruling 2026-09-01, issue #275; the tag-driven selector path retires per `docs/runbooks/site-sync-branch-flip.md`) and two direct `prune: false`, `deletionPolicy: Orphan` reconcilers. Issue #252 advanced that selection to the sites' published releases; issues #254, #258 and #268 advanced naranjo.online alone through `0.1.62`, `0.1.63` and `0.1.66` while lidersea.com stayed `0.1.40`; the issues #280/#281 drift remediation advanced both sites to `0.1.69` and `0.1.41`; and the issue #285 remediation advanced naranjo.online alone to `0.1.71` while re-verifying lidersea.com unchanged, each step on acquisition-receipt evidence alone; acquisition receipts never assert live convergence, so no health claim of any kind attaches to the committed digests. No live equivalence, readiness, or traffic is claimed |
